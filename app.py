...
TILE_INFO = {
    ...
    'zhong': {'name': '中', 'icon': ' 🀄︎ ︎', 'w': 35, 'type': 'z'}, 'fa': {'name': '發', 'icon': ' 🀅 ', 'w': 36, 'type': 'z'},
...
# --- 🌟 LLM 智慧教練模組 (專屬麻將助手) 🌟 ---
def get_majiang_coach_advice(hand_codes, exp_codes):
    try:
        api_key = st.secrets["GEMINI_API_KEY"]
        genai.configure(api_key=api_key)
    except KeyError:
        return "⚠️ 系統錯誤：找不到 API Key，請至 Streamlit Cloud 的 Settings -> Secrets 中設定 `GEMINI_API_KEY`。"
...
def render_main_ui(mode):
    ...
    #  🔹  聽牌分析模式介面  🔹
        ...
        if status == "waiting":
            ...
            html_content = f"""<div class="result-box" style="background-color: {bg_color}; color: {text_color};"><div class="result-title"> 👀  聽牌分析</div><div class="result-content"> 🔥  {title}</div><div style="margin-top: 10px;
            font-size: 20px;">這手牌聽以下這些牌：</div><div class="waiting-tiles-container">{icon_html}</div></div>"""
    ...
    #  🔹  🌟 修改 4：全新「麻將助手」介面 🌟  🔹
        ...
        if st.button("💡 獲取下一步捨牌建議", use_container_width=True, type="primary", key="btn_ast"):
            with st.spinner("教練正在分析最佳打法..."):
                ...
                advice = get_majiang_coach_advice(st.session_state.con_manual, st.session_state.exp_manual)
